...
NSString* const MFSideMenuStateNotificationEvent =
    @"MFSideMenuStateNotificationEvent";
...
containerWithCenterViewController:(id)centerViewController
  ...
  MFSideMenuContainerViewController* controller =
      [MFSideMenuContainerViewController new];
...
- (void)setDefaultSettings {
  ...
  self.menuWidth = [UIScreen mainScreen].bounds.size.width - 50.0;
...
- (void)setLeftMenuViewController:
  ...
  if (self.menuContainerView.superview) {
    [self.menuContainerView insertSubview:[_leftSideMenuViewController view]
                                  atIndex:0];
  }
...
- (void)setCenterViewController:(UIViewController*)centerViewController {
  ...
  [((UIViewController*)_centerViewController)view].frame =
      (CGRect){.origin = origin, .size = centerViewController.view.frame.size};
...
- (void)setRightMenuViewController:
  ...
  if (self.menuContainerView.superview) {
    [self.menuContainerView insertSubview:[_rightSideMenuViewController view]
                                  atIndex:0];
  }
...
- (void)removeChildViewControllerFromContainer:
...
- (void)removeCenterGestureRecognizers {
  if (self.centerViewController) {
    [[self.centerViewController view]
        removeGestureRecognizer:[self centerTapGestureRecognizer]];
    [[self.centerViewController view]
        removeGestureRecognizer:[self panGestureRecognizer]];
  }
}
...
- (void)addCenterGestureRecognizers {
  if (self.centerViewController) {
    [[self.centerViewController view]
        addGestureRecognizer:[self centerTapGestureRecognizer]];
    [[self.centerViewController view]
...
- (void)openLeftSideMenuCompletion:(void (^)(void))completion {
  ...
  [self.menuContainerView
      bringSubviewToFront:[self.leftMenuViewController view]];
  [self setCenterViewControllerOffset:self.leftMenuWidth
                             animated:YES
...
- (void)openRightSideMenuCompletion:(void (^)(void))completion {
  if (!self.rightMenuViewController)
    return;
  [self.menuContainerView
      bringSubviewToFront:[self.rightMenuViewController view]];
  [self setCenterViewControllerOffset:-1 * self.rightMenuWidth
                             animated:YES
                           completion:completion];
}
...
- (void)setMenuState:(MFSideMenuState)menuState
  ...
  void (^innerCompletion)() = ^{
    ...
    MFSideMenuStateEvent eventType = (_menuState == MFSideMenuStateClosed)
                                         ? MFSideMenuStateEventMenuDidClose
...
// these callbacks are called when the menu will become visible, not
// neccessarily when they will OPEN
...
- (void)setLeftSideMenuFrameToClosedPosition {
  ...
  leftFrame.origin.x =
      (self.menuSlideAnimationEnabled)
          ? -1 * leftFrame.size.width / self.menuSlideAnimationFactor
          : 0;
  leftFrame.origin.y = 0;
  [self.leftMenuViewController view].frame = leftFrame;
  [self.leftMenuViewController view].autoresizingMask =
...
- (void)setRightSideMenuFrameToClosedPosition {
  ...
  rightFrame.origin.x =
      self.menuContainerView.frame.size.width - self.rightMenuWidth;
  ...
  [self.rightMenuViewController view].autoresizingMask =
...
- (void)alignLeftMenuControllerWithCenterViewController {
  ...
  CGFloat xPositionDivider =
      (self.menuSlideAnimationEnabled) ? self.menuSlideAnimationFactor : 1.0;
  leftMenuFrame.origin.x =
      xOffset / xPositionDivider - _leftMenuWidth / xPositionDivider;
...
- (void)alignRightMenuControllerWithCenterViewController {
  ...
  CGFloat xPositionDivider =
      (self.menuSlideAnimationEnabled) ? self.menuSlideAnimationFactor : 1.0;
  rightMenuFrame.origin.x = self.menuContainerView.frame.size.width -
                            _rightMenuWidth + xOffset / xPositionDivider +
...
- (BOOL)centerViewControllerPanEnabled {
  return ((self.panMode & MFSideMenuPanModeCenterViewController) ==
          MFSideMenuPanModeCenterViewController);
}

- (BOOL)sideMenuPanEnabled {
  return ((self.panMode & MFSideMenuPanModeSideMenu) ==
...
- (void)handleRightPan:(UIPanGestureRecognizer*)recognizer {
  ...
  if (self.menuState == MFSideMenuStateRightMenuOpen) {
    // menu is already open, the most the user can do is close it in this
    // gesture
  ...
  if (recognizer.state == UIGestureRecognizerStateEnded) {
    ...
    if (self.menuState == MFSideMenuStateClosed) {
      BOOL showMenu =
          (finalX > viewWidth / 2) || (finalX > self.leftMenuWidth / 2);
      if (showMenu) {
  ...
  if (recognizer.state == UIGestureRecognizerStateEnded) {
    ...
    if (self.menuState == MFSideMenuStateClosed) {
      BOOL showMenu = (finalX < -1 * viewWidth / 2) ||
                      (finalX < -1 * self.rightMenuWidth / 2);
      if (showMenu) {
        self.panGestureVelocity = velocity.x;
...
- (void)setCenterViewControllerOffset:(CGFloat)offset
  ...
  if (animated) {
    CGFloat centerViewControllerXPosition =
        ABS([self.centerViewController view].frame.origin.x);
    CGFloat duration =
        [self animationDurationFromStartPosition:centerViewControllerXPosition
                                   toEndPosition:offset];
...
- (CGFloat)animationDurationFromStartPosition:(CGFloat)startPosition
  ...
  if (ABS(self.panGestureVelocity) > 1.0) {
    ...
    CGFloat animationPerecent =
        (animationPositionDelta == 0) ? 0 : menuWidth / animationPositionDelta;
...
- (void)addCenterShadowView:(UIViewController*)centerViewController {
  if (self.centerShadowEnabled) {
    self.centerShadowView = [[UIView alloc]
        initWithFrame:CGRectMake(0, 0,
                                 centerViewController.view.frame.size.width,
                                 centerViewController.view.frame.size.height)];
