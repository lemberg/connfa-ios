...
{
    NSArray *eventClasses = @[[DCMainEvent class], [DCBof class], [DCSocialEvent class]];

    NSMutableArray *sections = [NSMutableArray array];
    for (Class class in eventClasses) {
        NSArray *uniqueTimeSlotForDay = [self uniqueTimeRangesForDay:self.selectedDay andEventClass:class];
...
{
    return [[DCMainProxy sharedProxy] eventsForDay:day forClass:eventClass predicate:self.eventStrategy.predicate];
}

- (NSArray*)uniqueTimeRangesForDay:(NSDate*)day andEventClass:(Class)eventClass
{
    return [[DCMainProxy sharedProxy] uniqueTimeRangesForDay:day forClass:eventClass predicate:self.eventStrategy.predicate];
}
