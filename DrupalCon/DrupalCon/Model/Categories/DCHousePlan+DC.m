...
+ (void)updateFromDictionary:(NSDictionary *)housePlans inContext:(NSManagedObjectContext *)context
{
    //adding
    for (NSDictionary *dictionary in housePlans[kDCHousePlansKey]) {
        DCHousePlan * housePlan = (DCHousePlan*)[[DCMainProxy sharedProxy] objectForID:[dictionary[kDCHousePlanIdKey] intValue] ofClass:[DCHousePlan class] inContext:context];
        ...
        else // update
        {
            ...
            housePlan.order = [NSNumber numberWithFloat:[dictionary[kDCParseObjectOrderKey] floatValue]];
        }
    }
}
