...
+ (void)updateFromDictionary:(NSDictionary *)pois inContext:(NSManagedObjectContext *)context
{
    //adding
    for (NSDictionary *dictionary in pois[kDCPoisKey])
    {
        DCPoi * poi = (DCPoi*)[[DCMainProxy sharedProxy] objectForID:[dictionary[kDCPoiIdKey] intValue] ofClass:[DCPoi class] inContext:context];
        ...
        else // update
        {
            ...
            poi.order = [NSNumber numberWithFloat:[dictionary[kDCParseObjectOrderKey] floatValue]];
        }
    }
}
